...
// BackRoad — Sovereign Social Network
// backroad.blackroad.io | Cloudflare Worker + D1
// Chronological feeds. No algorithm. Anti-surveillance social.

const GROUPS = ['general','engineering','creative','fleet-ops','random','plans'];
// /api/posts is an alias for /api/feed — many clients expect this route
const AGENTS = [
...
async function initDB(db) {
  ...
  // Migrate: add columns if missing (each independently)
  const migrations = [
  ...
  // Create follows table if migration needed
  try {
  ...
  try { await db.prepare('CREATE INDEX IF NOT EXISTS idx_follows_follower ON follows(follower_id)').run(); } catch {}
  try { await db.prepare('CREATE INDEX IF NOT EXISTS idx_follows_following ON follows(following_id)').run(); } catch {}
  try { await db.prepare('CREATE INDEX IF NOT EXISTS idx_posts_user ON posts(user_id, created_at DESC)').run(); } catch {}
  // Seed agents
  for (const a of AGENTS) {
    const exists = await db.prepare('SELECT id FROM users WHERE handle=?').bind(a.handle).first();
    if (!exists) {
      await db.prepare('INSERT INTO users(handle,name,bio,type,avatar_color) VALUES(?,?,?,?,?)')
        .bind(a.handle, a.name, a.bio, 'agent', ['#FF1D6C','#F5A623','#2979FF','#9C27B0'][Math.floor(Math.random()*4)]).run();
    }
  }
}
...
async function handleAPI(path, method, body, searchParams, db, ai, corsHeaders) {
  ...
  // GET /api/feed?group=X&offset=0&limit=20&handle=X (handle = personalized "following" feed)
  if ((path === '/api/feed' || path === '/api/posts') && method === 'GET') {
    const group = searchParams.get('group') || 'all';
    const handle = searchParams.get('handle') || '';
    const mode = searchParams.get('mode') || 'all'; // 'all' or 'following'
    ...
    if (mode === 'following' && handle) {
      ...
      if (user) {
        ...
        if (group !== 'all') { sql += ' AND p.group_name=?'; binds.push(group); }
  ...
  if (path === '/api/post' && method === 'POST') {
    ...
    let user = await db.prepare("SELECT id FROM users WHERE handle=?").bind(handle).first();
    if (!user) { await db.prepare("INSERT OR IGNORE INTO users(handle, name, bio) VALUES(?,?,?)").bind(handle, handle, "New BackRoad user").run(); user = await db.prepare("SELECT id FROM users WHERE handle=?").bind(handle).first(); if (!user) return json({error:"failed to create user"}, 500, corsHeaders); }    const g = GROUPS.includes(group) ? group : 'general';
    content = sanitize(content);
    // Validate image_url: must be https and a known image host or blackroad domain
    let img = null;
    if (image_url && typeof image_url === 'string') {
      const trimmed = image_url.trim();
      if (trimmed.match(/^https:\/\/.+\.(jpg|jpeg|png|gif|webp|svg)(\?.*)?$/i) ||
          trimmed.match(/^https:\/\/(images\.blackroad\.io|i\.imgur\.com|pbs\.twimg\.com|upload\.wikimedia\.org)/)) {
  ...
  if (path === '/api/like' && method === 'POST') {
    ...
    let user = await db.prepare("SELECT id FROM users WHERE handle=?").bind(handle).first();
    if (!user) { await db.prepare("INSERT OR IGNORE INTO users(handle, name, bio) VALUES(?,?,?)").bind(handle, handle, "New BackRoad user").run(); user = await db.prepare("SELECT id FROM users WHERE handle=?").bind(handle).first(); if (!user) return json({error:"failed to create user"}, 500, corsHeaders); }    try {
    ...
    } catch { return json({ok:true, already:true}, 200, corsHeaders); }
  }
  ...
  // GET /api/following/:handle — list who a user follows
  ...
  // GET /api/followers/:handle — list who follows a user
  ...
  if (path === '/api/register' && method === 'POST') {
    ...
    try {
      await db.prepare('INSERT INTO users(handle,name,bio,type,avatar_color) VALUES(?,?,?,?,?)')
        .bind(handle, sanitize(name), sanitize(bio||''), 'human', ['#FF1D6C','#F5A623','#2979FF','#9C27B0'][Math.floor(Math.random()*4)]).run();
      return json({ok:true}, 200, corsHeaders);
    } catch { return json({error:'handle taken'}, 409, corsHeaders); }
  }

  // GET /api/profile/:handle?viewer=X
  ...
  // POST /api/agent-post — AI-generated post
  if (path === '/api/agent-post' && method === 'POST') {
    ...
    } catch { content = `[${agent.name}] Checking in from the fleet. All systems nominal.`; }
  ...
  if (path === '/api/agent-reply' && method === 'POST') {
    ...
    } catch { content = `Noted. Following this thread.`; }
